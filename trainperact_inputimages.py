...
def prepare_batch(batch_data, cfg, lang_embed_cache, device):
    ...
    for k, v in obs_dict.items():
        ...
        # The code for saving the input images for the per Act model
        for k, v in obs_dict.items():
            if 'rgb' in k:
                for i, img in enumerate(v):
                    if img.shape[0] == 3: # Check if the image has 3 channels (assuming it's in the format [C, H, W])
                        img = img.transpose(1, 2, 0) # Transpose the image to the format [H, W, C]
                    plt.imshow(img)
                    plt.savefig(f'/media/local/atarek/Inputimages/{k}_image_{i}.png') # Save the image with a filename based on the key and index
                    plt.close()
