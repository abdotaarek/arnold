...
class PickupObject(BaseTask):
    ...
    def save_frames(self, frame_number, render_output, state_description):
        save_directory = '/media/local/atarek/3Dscene/test'
        os.makedirs(save_directory, exist_ok=True)

        camera_types = ['gripper_cam', 'base_cam', 'cam_3', 'cam_4', 'cam_5']  # Update as per your camera names

        for i, camera_output in enumerate(render_output['images']):
            rgb_data = camera_output['rgb']
            image = Image.fromarray(rgb_data)
            camera_type = camera_types[i]
            filename = f"ep_{self.episode_number}_{state_description}_{camera_type}_frame_{frame_number:05d}.png"
    ...
    def step(self, act_pos, act_rot, render, use_gt):
        ...
        if self.current_stage == 0 and self.time_step == 0:
            render_output = self.render()
            self.save_frames(self.frame_number, render_output, "initial")
            self.frame_number += 1
        ...
        if self.current_stage == self.num_stages:
            render_output = self.render()
            self.save_frames(self.frame_number, render_output, "end")
            self.frame_number += 1
            self.episode_number += 1  # Increment the episode counter
