...
class OpenDrawer(BaseTask):
    def __init__(self, num_stages, horizon, stage_properties, cfg, camera_view=0) -> None:
        ...
        self.frame_number = 0
        self.camera_view_count = camera_view
    #saves frames from all cameraviews
    def save_frame(self, frame_number, save_base_directory=Path('/media/local/atarek/3Dscene/test')):
        try:
            render_output = self.render()

            for camera_view in range(len(render_output['images'])):
                rgb_data = render_output['images'][camera_view]['rgb']
                image = Image.fromarray(rgb_data)

                camera_directory = save_base_directory / f"camera_{camera_view}"
                camera_directory.mkdir(parents=True, exist_ok=True)

                filename = f"frame_{frame_number:05d}.png"
                image_path = camera_directory / filename
