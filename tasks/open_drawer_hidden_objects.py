...
class OpenDrawer(BaseTask):
    def __init__(self, num_stages, horizon, stage_properties, cfg, camera_view=2) -> None:
    ...
    def save_frame(self, frame_number, save_directory=Path('/media/local/atarek/3Dscene/test')):
        try:
    ...
    def set_up_task(self) -> None:
        self.load_object()
    # prints all the names of the objects as a USDpaths
    def print_all_prims_on_stage(self):
        print("Listing all prims in the stage:")
        if self.stage:
    ...
    def load_object(self):
        ...
        objects_to_hide_paths = ["/World_0/house/furniture", "/World_0/house/doors", "/World_0/house/windows", "/World_0/house/floors"] # write any object you want to hide
        self.print_all_prims_on_stage()
        from omni.isaac.core.utils.prims import get_prim_at_path
        for object_path in objects_to_hide_paths:
            try:
                prim = get_prim_at_path(object_path)
                if prim:
                    print(f"Hiding object at path: {object_path}")
                    prim.SetActive(False)
                else:
                    print(f"Object not found at path: {object_path}")
            except Exception as e:
                print(f"Error hiding object at path {object_path}: {e}")
    ...
    def step(self, act_pos, act_rot, render, use_gt):
        ...
        while self.current_stage < self.end_stage:
            ...
            simulation_context.step(render=render)
            self.time_step += 1
            self.save_frame(self.frame_number)
            self.frame_number += 1
            self.time_step += 1
